...
run "group_events_sizing_can_be_raised" {
  ...
  assert {
    condition     = var.group_events.timeout == 120 && var.group_events.memory_size == 512
    error_message = "A 3-second budget to drain a batch and republish is tight; it must be raisable"
  }
}
