...
output "dead_letter_queue_arns" {
  ...
  value = compact([
    local.group_events ? module.oxbow_fifo_queue[0].dead_letter_queue_arn : module.oxbow_queue[0].dead_letter_queue_arn,
    local.group_events ? module.group_events_queue[0].dead_letter_queue_arn : "",
    var.enable_auto_tagging ? module.auto_tagging_queue[0].dead_letter_queue_arn : "",
    var.enable_glue_create ? module.glue_create_queue[0].dead_letter_queue_arn : "",
    var.enable_glue_sync ? module.glue_sync_queue[0].dead_letter_queue_arn : "",
  ])
}

output "autotag_sqs_arn" {
  description = "SQS ARN for the auto-tagging lambda"
  value       = var.enable_auto_tagging ? module.auto_tagging_queue[0].queue_arn : ""
}

output "autotag_lambda" {
  description = "Auto-tagging lambda ARN"
  value       = var.enable_auto_tagging ? module.auto_tagging_lambda[0].lambda_function_arn : ""
}
